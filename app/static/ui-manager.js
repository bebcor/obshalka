    ...
    _updateVideoOverlaysInternal() {
        ...
        this.videoCallManager.remoteStreams.forEach((stream, userId) => {
            ...
            if (!participantCard) {
                ...
                if (peerConnection) {
                    ...
                    if (videoReceiver && videoReceiver.track) {
                        ...
                        console.log(`✅ [updateVideoOverlays ${userId}] Создаем карточку - обнаружен активный видео трек (enabled=true)`);
                        this.createRemoteVideoElement(userId, stream);
                        participantCard = document.getElementById(`participant-${userId}`);
                        if (!participantCard) {
                            console.error(`❌ [updateVideoOverlays ${userId}] Не удалось создать карточку`);
                            return;
                        }
                    } else {
                        // Нет активного видео трека - не создаем карточку
                        console.log(`❌ [updateVideoOverlays ${userId}] НЕ создаем карточку - нет активного видео трека (disabled)`);
                        return;
                    }
                } else {
                    // Нет peer connection - не создаем карточку
                    return;
                }
            }
            ...
            if (peerConnection) {
                ...
                // 4. Трек muted=true (камера выключена пользователем)
            ...
            if (peerConnection) {
                ...
                if (hasNullVideoReceiver) {
                    ...
                    streamVideoTracks.forEach(streamTrack => {
                        ...
                        } else if (receiverTrack && !receiverTrack.enabled) {
                ...
                if (streamTracksCount === 0 && receivers.length > 0) {
                    ...
                    receivers.forEach((receiver, index) => {
                        ...
                        if (track && track.readyState === 'live') {
                            // Для видео: добавляем ТОЛЬКО если live И enabled (камера включена)
                            // ВАЖНО: muted=true означает что источник недоступен - НЕ добавляем такой трек
                            if (track.kind === 'video') {
                                // КРИТИЧНО: Если enabled=false ИЛИ muted=true, камера выключена - не добавляем трек
                                if (track.enabled && !track.muted) {
                                    ...
                                    console.log(`❌ [updateVideoOverlays ${userId}] НЕ добавляем видео трек ${track.id} из receiver ${index} - enabled=${track.enabled}, muted=${track.muted} (камера выключена)`);
                                }
                ...
                if (!videoReceiver) {
                    ...
                    // ВАЖНО: Если enabled=false, камера выключена пользователем - не показываем карточку
                    // muted - это временное состояние при инициализации, НЕ используем его
                    const isActive = track.enabled && track.readyState === 'live';
                    hasActiveVideo = isActive; // hasActiveVideo = true только если трек активен (enabled=true)
                    
                    // КРИТИЧНО: Добавляем трек в поток ТОЛЬКО если он enabled И НЕ muted
                    ...
                    if (!trackInStream && track.enabled && track.readyState === 'live' && !track.muted) {
                        console.log(`✅ [${userId}] Добавляем видео трек из receivers в поток (enabled=${track.enabled}, muted=${track.muted})`);
                        stream.addTrack(track);
                    } else if (!trackInStream) {
                        console.log(`❌ [${userId}] НЕ добавляем видео трек в поток - камера выключена (enabled=${track.enabled}, muted=${track.muted}, readyState=${track.readyState})`);
                    }
                }
            } else {
            ...
            // ВАЖНО: Если enabled=false, камера выключена пользователем - не добавляем трек
            // muted - это временное состояние при инициализации, НЕ используем его
            if (hasActiveVideo && !hasActiveVideoInStream && peerConnection) {
                const receivers = peerConnection.getReceivers();
                const videoReceiver = receivers.find(receiver => {
                    const track = receiver.track;
                    // КРИТИЧНО: Проверяем что трек enabled И НЕ muted
                ...
                if (videoReceiver && videoReceiver.track) {
                    ...
                    if (track.enabled && !track.muted) {
                        const trackInStream = stream.getTracks().find(t => t.id === track.id);
                        if (!trackInStream) {
                            console.log(`✅ [updateVideoOverlays ${userId}] Добавляем активный видео трек из receivers в поток (финальная проверка, enabled=${track.enabled}, muted=${track.muted})`);
                            stream.addTrack(track);
                    ...
                    if (videoReceiver && videoReceiver.track) {
                        // Есть активный receiver, но его нет в потоке - добавляем ТОЛЬКО если enabled И НЕ muted
                        const track = videoReceiver.track;
                        if (track.enabled && !track.muted) {
                            console.log(`✅ [updateVideoOverlays ${userId}] Добавляем активный видео трек из receivers в поток (нет в потоке, enabled=${track.enabled}, muted=${track.muted})`);
                            stream.addTrack(track);
                            hasActiveVideo = true;
                        }
                    } else {
            ...
            // ДОПОЛНИТЕЛЬНО: Проверяем, что треки действительно активны (enabled, live)
            // ВАЖНО: Если enabled=false, камера выключена пользователем - не показываем карточку
            // muted - это временное состояние при инициализации, НЕ используем его
            ...
            // КРИТИЧНО: Проверяем также активность треков в потоке (enabled, live)
            // КРИТИЧНО: muted не влияет на активность - это временное состояние браузера
            ...
            // КРИТИЧНО: Показываем карточку ТОЛЬКО если есть АКТИВНЫЙ видео трек (enabled=true)
            // ВАЖНО: Если в потоке есть только disabled треки, удаляем карточку
            const hasDisabledVideoInStream = finalVideoTracks.some(t => t && !t.enabled);
            if (hasDisabledVideoInStream && !finalHasActiveVideo) {
                // В потоке есть только disabled треки - удаляем карточку
                console.log(`❌ [updateVideoOverlays ${userId}] В потоке есть только disabled треки - удаляем карточку`);
                if (videoElement) {
                    videoElement.style.setProperty('display', 'none', 'important');
                    videoElement.pause();
                    videoElement.srcObject = null;
            ...
            if (hasActiveVideoTrackInReceivers && finalHasActiveVideo) {
                // ВАЖНО: Если карточка не существует, создаем ее
                if (!participantCard) {
                    console.log(`✅ [updateVideoOverlays ${userId}] Создаем карточку - появился активный видео трек`);
                    this.createRemoteVideoElement(userId, stream);
                    // После создания карточки нужно получить ее снова
                    const newParticipantCard = document.getElementById(`participant-${userId}`);
                    const newVideoElement = document.getElementById(`remoteVideo-${userId}`);
                    if (!newParticipantCard || !newVideoElement) {
                        console.error(`❌ [updateVideoOverlays ${userId}] Не удалось создать карточку`);
                        return;
                    }
                    // Используем новые элементы
                    participantCard = newParticipantCard;
                    videoElement = newVideoElement;
                }
                
                // КРИТИЧНО: Перед показом карточки проверяем, что в потоке нет disabled треков
                // Если есть disabled треки, удаляем их и карточку
                const disabledVideoTracksInStream = stream.getVideoTracks().filter(t => t && !t.enabled);
                if (disabledVideoTracksInStream.length > 0) {
                    console.log(`❌ [updateVideoOverlays ${userId}] В потоке есть disabled треки - удаляем их и карточку`);
                    disabledVideoTracksInStream.forEach(track => {
                        stream.removeTrack(track);
                    });
                    if (videoElement) {
                        videoElement.style.setProperty('display', 'none', 'important');
                ...
                if (videoElement) {
                    ...
                    // КРИТИЧНО: Проверяем, что в потоке есть АКТИВНЫЕ видео треки (enabled=true)
                    // muted - это временное состояние при инициализации, НЕ используем его
            ...
            if (peerConnection) {
                ...
                const videoReceiver = receivers.find(receiver => {
                    const track = receiver.track;
                    // ВАЖНО: Если muted=true, камера выключена - не показываем карточку
                    return track && track.kind === 'video' && track.readyState === 'live' && track.enabled && !track.muted;
                });
                if (videoReceiver && videoReceiver.track) {
                    const track = videoReceiver.track;
                    ...
                    if (track.enabled && !track.muted) {
                        const trackInStream = stream.getTracks().find(t => t.id === track.id);
                        if (!trackInStream) {
                            console.log(`✅ [updateVideoOverlays ${userId}] НАЙДЕН активный видео трек в receivers при финальной проверке, добавляем в поток (enabled=${track.enabled}, muted=${track.muted})`);
                            stream.addTrack(track);
            ...
            // НЕТ активного видео - УДАЛЯЕМ карточку из DOM
            ...
            // Дополнительная проверка: если трек disabled, тоже удаляем карточку
            const hasDisabledVideo = peerConnection && peerConnection.getReceivers().some(r => {
                const track = r.track;
                return track && track.kind === 'video' && !track.enabled;
            });
            
            if (hasDisabledVideo) {
                console.log(`❌ [updateVideoOverlays ${userId}] Видео трек disabled - удаляем карточку из DOM`);
            } else {
                console.log(`❌ [updateVideoOverlays ${userId}] НЕТ активного видео - удаляем карточку из DOM`);
            }
            
            // КРИТИЧНО: СНАЧАЛА очищаем srcObject и останавливаем воспроизведение
            ...
            // КРИТИЧНО: Удаляем карточку из DOM полностью, если она существует
            if (participantCard && participantCard.parentNode) {
                console.log(`🗑️ [updateVideoOverlays ${userId}] Удаляем карточку из DOM (нет активного видео)`);
                // Останавливаем интервал проверки треков если он есть
                if (this.trackCheckIntervals && this.trackCheckIntervals.has(userId)) {
                    clearInterval(this.trackCheckIntervals.get(userId));
                    this.trackCheckIntervals.delete(userId);
                }
        ...
        if (videoElement) {
            ...
            stream.addTrack = (track) => {
                ...
                if (track.kind === 'video' && (!track.enabled || track.readyState !== 'live')) {
                    console.log(`⚠️ Видео трек добавлен как неактивный для ${userId}, muted: ${track.muted}, enabled: ${track.enabled}, readyState: ${track.readyState}`);
                    // Ждем 500мс и перепроверяем состояние трека
                    setTimeout(() => {
                        const currentTrack = stream.getVideoTracks().find(t => t.id === track.id);
                        if (currentTrack) {
                            const isNowActive = currentTrack.enabled && currentTrack.readyState === 'live';

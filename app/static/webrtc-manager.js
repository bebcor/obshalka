        ...
        try {
            ...
            peerConnection.onconnectionstatechange = () => {
                ...
                if (state === 'connected') {
                    ...
                    setTimeout(() => {
                        console.log(`🔄 [ConnectionState] Обновляем UI после установки соединения для ${targetUserId}`);
                        this.videoCallManager.uiManager.updateVideoOverlays();
                    }, 100);
            ...
            peerConnection.oniceconnectionstatechange = () => {
                ...
                if (iceState === 'connected' || iceState === 'completed') {
                    ...
                    // Небольшая задержка чтобы дать время состоянию обновиться
                    setTimeout(() => {
                        console.log(`🔄 [ICEConnectionState] Обновляем UI после установки ICE соединения для ${targetUserId}`);
                        this.videoCallManager.uiManager.updateVideoOverlays();
                    }, 100);
        ...
        const checkAndSync = (delay) => {
            setTimeout(() => {
                ...
                receivers.forEach((receiver, index) => {
                    ...
                    // Если muted=true - камера выключена, данные не приходят, карточку НЕ создаем
                    if (track.readyState === 'live' && track.enabled && !track.muted) {
                        // ВАЖНО: Создаем карточку только когда появляется активный видео трек (enabled=true И НЕ muted)
                        if (track.kind === 'video') {
